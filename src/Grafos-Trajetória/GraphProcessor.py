...
bolas espacialmente, acumula contagens em N faixas horizontais que se
...
def construir_faixas(poligono: Polygon, n: int) -> list[FaixaQuadra]:
    y_min, y_max = poligono.bounds[1], poligono.bounds[3]
    altura = (y_max - y_min) / n
    faixas = []
    for i in range(n):
        y0 = y_min + i * altura
        y1 = y_min + (i + 1) * altura
        faixas.append(FaixaQuadra(i, y0, y1, poligono))
    log("HUMANO", f"Quadra dividida em {n} faixas horizontais.")
    log("DEBUG",  f"altura por faixa = {altura*100:.1f}cm")
    for f in faixas:
...
# ─────────────────────────────────────────────
class BolaConhecida:
    __slots__ = ("x", "y", "n_obs", "ultima_visao", "faixa_id")
    def __init__(self, x, y, faixa_id):
        self.x = x
        self.y = y
        self.n_obs = 1
        self.ultima_visao = time.time()
    ...
    with estado.lock:
        for b in bolas_novas:
            ...
            if melhor is not None and melhor_d <= RAIO_DEDUP_M:
                # atualizar com média acumulativa (tira tremor)
                n = melhor.n_obs
                melhor.x = (melhor.x * n + x) / (n + 1)
                melhor.y = (melhor.y * n + y) / (n + 1)
                melhor.n_obs = n + 1
                melhor.ultima_visao = time.time()
    ...
    with estado.lock:
        ...
        for bc in estado.bolas:
            if 0 <= bc.faixa_id < len(estado.faixas):
                ...
                if not f.em_execucao and bc.n_obs >= N_OBS_MIN_ESTAVEL:
                    # Só bolas suficientemente "paradas" (vistas ≥ N_OBS_MIN_ESTAVEL
                    # frames consecutivos) contam para o peso da faixa. Bolas em
                    # movimento têm n_obs baixo porque caem fora do raio de
                    # deduplicação a cada novo frame.
                    f.peso += 1
...
def broadcast_estado(estado: "EstadoGrafo"):
    ...
    # snapshot fora do lock do broadcast (só com lock do estado)
    with estado.lock:
        robo = dict(estado.robo)
        fase = estado.fase_varrimento
        faixa_label = None
        alvo_destino = None
        if estado.disparo_ativo is not None:
    ...
    pacote = {
        "timestamp":     time.time(),
        "robo":          robo,
        "fase":          fase,           # None | "aguarda_inicio" | "em_varrimento"
        "faixa_label":   faixa_label,
        "alvo_destino":  alvo_destino,   # None se em idle
    }
...
class VisualizadorAoVivo:
    ...
    def atualizar(self):
        with self.estado.lock:
            ...
            bolas  = [(bc.x, bc.y, bc.faixa_id, bc.n_obs >= N_OBS_MIN_ESTAVEL)
        ...
        # Trajetória de 3 pontos (se disparo ativo)
        ...
        self.ax.set_title(
            ...
            f"peso={total}  |  threshold={int(THRESHOLD_PCT*100)}% & ≥{K_MIN}",
...
def processar_pacote(estado: EstadoGrafo, pacote: dict):
    ...
    if em_disparo:
        # Durante um disparo, NÃO acumulamos novas bolas (o robô em movimento
        # pode gerar deteções ruidosas). Apenas avançamos a máquina de
        # estados de varrimento com base na posição do robô.
        verificar_progresso_varrimento(estado)
    else:
        deduplicar_e_atualizar(estado, bolas)
        recalcular_pesos(estado)

    # Empurrar estado para o RobotController (e outros clientes) — sempre,
    # independentemente de haver disparo ou não, porque o controlador
    # precisa de saber quando o robô está em idle para parar os motores.
...
def main():
    ...
    global TIMEOUT_VARRIMENTO_S, _CFG
    ...
    N_FAIXAS                       = int(_CFG.get("n_faixas", N_FAIXAS))
    ...
    faixas   = construir_faixas(poligono, N_FAIXAS)
    estado   = EstadoGrafo(faixas, poligono, calib)

    log("DEBUG", f"Configuração: N={N_FAIXAS} | raio={RAIO_DEDUP_M*100:.1f}cm | "
                f"threshold={int(THRESHOLD_PCT*100)}% | K_min={K_MIN} | "
                f"n_obs_min={N_OBS_MIN_ESTAVEL}")
                ...
                if not em_disparo:
                    ...
                    if faixa is not None:
                        with estado.lock:
                            ...
                            estado._avisou_alinhamento = False
                            # Se o robô já está perto do início E alinhado,
                            # vai logo para "em_varrimento". Caso contrário,
                            # começa em "aguarda_inicio".
                            centro = _centro_robo(estado.robo)
                            v_robo = _vetor_robo(estado.robo)
                            ja_la = False
