...
const float MAX_VISION_TURN_STEP_DEG = 10.0;

// Sentido usado quando o servidor manda erro angular pela visao.
// Se o robo corrigir para o lado errado, trocar para -1.
...
const char* WIFI_SSID = "LMM";
const char* WIFI_PASS = "mecatronica";

const uint16_t UDP_CMD_PORT = 5005;  // servidor -> ESP32
const uint16_t UDP_FB_PORT = 5006;   // ESP32 -> servidor

IPAddress serverIp(192, 168, 1, 100);  // fallback; atualizado pelo primeiro pacote UDP recebido
WiFiUDP udp;
String segmentId;
long lastUdpSeq = -1;
...
void runFullTrajectory(JsonArray waypoints, float startX, float startY,
  ...
  float assumedHeading = startHeadingDeg;
  int total = waypoints.size();
  int index = 0;

  Serial.println("trajectory_full_start");
  Serial.print("start_pose=");
  Serial.print(startX, 3);
  Serial.print(",");
  Serial.print(startY, 3);
  Serial.print(" heading_deg=");
  Serial.println(startHeadingDeg, 2);
  Serial.print("waypoints=");
  Serial.println(total);

  for (JsonObject waypoint : waypoints) {
    index++;
    float targetX = waypoint["x"] | currentX;
    float targetY = waypoint["y"] | currentY;
    float dx = targetX - currentX;
    float dy = targetY - currentY;
    float distanceM = sqrt(dx * dx + dy * dy);
    float desiredHeading = atan2(dy, dx) * 180.0 / PI;
    float visionDelta = wrapDeg(desiredHeading - assumedHeading);
    float localTurn = wrapDeg(VISION_TURN_SIGN * visionDelta);

    Serial.print("trajectory_point=");
    Serial.print(index);
    Serial.print("/");
    Serial.print(total);
    Serial.print(" from=");
    ...
    Serial.print(distanceM * 100.0, 1);
    Serial.print(" desired_heading=");
    Serial.print(desiredHeading, 2);
    Serial.print(" delta_vision=");
    Serial.print(visionDelta, 2);
    Serial.print(" turn_local=");
    Serial.println(localTurn, 1);
    ...
    currentX = targetX;
    currentY = targetY;
    assumedHeading = desiredHeading;
    delay(350);
  }

  stopMotors();
  Serial.println("trajectory_full_end");
  udpEvent("trajectory_done");
}
...
void udpPoll() {
  ...
  if (commandSeq >= 0 && commandSeq <= lastUdpSeq) {
    Serial.print("udp_stale_seq=");
    Serial.println(commandSeq);
    return;
  }
  if (commandSeq >= 0) lastUdpSeq = commandSeq;

  if (trajectoryId.length() > 0) {
    if (firstTrajectoryId.length() == 0) {
      firstTrajectoryId = trajectoryId;
      firstTrajectoryCalibration = true;
      Serial.print("first_trajectory_calibration=");
      Serial.println(firstTrajectoryId);
    } else if (trajectoryId != firstTrajectoryId && firstTrajectoryCalibration) {
      firstTrajectoryCalibration = false;
      Serial.println("first_trajectory_calibration_done");
    }
  }
  ...
  if (!strcmp(type, "trajectory_full")) {
    JsonArray waypoints = doc["waypoints"].as<JsonArray>();
    float startX = doc["start_pose"]["x"] | 0.0;
    float startY = doc["start_pose"]["y"] | 0.0;
    float startHeading = doc["start_pose"]["heading_deg"] | 0.0;

    if (waypoints.size() == 0) {
      Serial.println("trajectory_full_empty");
      udpEvent("trajectory_error");
      return;
    }
    runFullTrajectory(waypoints, startX, startY, startHeading);
  } else if (!strcmp(type, "orient_goal") || !strcmp(type, "orientation_correction")) {
    float visionHeading = doc["robot_pose"]["heading_deg"] | 0.0;
    float desiredHeading = doc["desired_heading_deg"] | visionHeading;
    float visionError = doc["heading_error_deg"] | wrapDeg(desiredHeading - visionHeading);
    float headingTolerance = doc["tolerance_heading_deg"] | 10.0;
    float correctionError = 0.0;
    if (abs(visionError) > headingTolerance) {
      float correctionMagnitude = abs(visionError) - headingTolerance * 0.5;
      correctionError = visionError > 0.0 ? correctionMagnitude : -correctionMagnitude;
    }
    ...
    Serial.print("vision_heading=");
    Serial.print(visionHeading, 2);
    Serial.print(" desired=");
    Serial.print(desiredHeading, 2);
    Serial.print(" error=");
    Serial.print(visionError, 2);
    Serial.print(" tolerance=");
    Serial.print(headingTolerance, 2);
    Serial.print(" correction_error=");
    ...
    stopMotors();
    udpEvent("stopped");
  } else if (!strcmp(type, "arrived_ok") || !strcmp(type, "arrived_bad")) {
    Serial.print("server_validation=");
    Serial.println(type);
  }
}
#endif
